...
type Tab = 'hoy' | 'cargar' | 'pagos' | 'pl' | 'mas'
...
export default function Dashboard() {
  ...
  // Data
  const [datosDia, setDatosDia] = useState<any>({efectivo:0,transferencias:0,tarjeta_pendiente:0,cheque_recibido:0,saldo_banco:0,ventas_acumuladas_mes:15000000,notas:''})
  ...
  async function logHistorial(accion: string, descripcion: string) {
  ...
  async function guardarDatos() {
    ...
    if (!error) {
      await logHistorial('UPDATE', `Actualizó datos del día — caja: ${fmtS(datosDia.efectivo + datosDia.transferencias + datosDia.saldo_banco)}`)
      setMsg('✓ Guardado')
  ...
  async function agregarVencimiento() {
    ...
    await logHistorial('INSERT', `Agregó vencimiento: ${fVenc.descripcion} — ${fmt(parseFloat(fVenc.monto))}`)
    setFVenc({fecha:'',descripcion:'',monto:'',tipo:'cheque'})
    setModal('')
    await loadAll()
  }

  async function marcarPagado(id: number, desc: string) {
    await supabase.from('vencimientos').update({pagado:true, fecha_pago: hoyStr()}).eq('id', id)
    await logHistorial('UPDATE', `Marcó como pagado: ${desc}`)
    await loadAll()
  }
  ...
  async function agregarDeuda() {
    ...
    await logHistorial('INSERT', `Agregó deuda: ${fDeuda.descripcion} — ${fmt(parseFloat(fDeuda.monto))}`)
  ...
  async function agregarGasto() {
    ...
    await logHistorial('INSERT', `Registró gasto: ${fGasto.descripcion} — ${fmt(parseFloat(fGasto.monto))}`)
  ...
  async function eliminarItem(tabla: string, id: number, desc: string) {
    if (!confirm(`¿Eliminar "${desc}"?`)) return
    await supabase.from(tabla).update({activa:false}).eq('id', id)
    await logHistorial('DELETE', `Eliminó de ${tabla}: ${desc}`)
    await loadAll()
  }
  ...
  const tv7 = v7.reduce((s: number, v: any) => s + v.monto, 0)
  const v15 = vencimientos.filter(v => { const d = diasHasta(v.fecha); return d >= 0 && d <= 15 })
  const tv15 = v15.reduce((s: number, v: any) => s + v.monto, 0)
  ...
  const neto = mb - FIJOS
  const stockValor = stock.reduce((s: number, i: any) => s + (i.cantidad * i.costo_unitario), 0)
  const totalDeudas = deudas.reduce((s: number, d: any) => s + d.monto, 0)

  const tipoBadge = (t: string) => {
    const m: any = {cheque:'#f5a623',banco:'#5b9fff',impuesto:'#ff5050',sueldo:'#ff5050',servicio:'#7a7876',tarjeta:'#f5a623',proveedor:'#7a7876',personal:'#5b9fff',stock:'#3ddc84',alquiler:'#5b9fff',otro:'#7a7876'}
    const l: any = {cheque:'CHQ',banco:'BCO',impuesto:'AFIP',sueldo:'SUE',servicio:'SVC',tarjeta:'TRJ',proveedor:'PRV',personal:'CRED',stock:'STK',alquiler:'ALQ',otro:'OTRO'}
    return <span style={{fontSize:'9px',fontWeight:700,padding:'2px 6px',borderRadius:'4px',fontFamily:'monospace',background:m[t]+'22',color:m[t]}}>{l[t]||t.toUpperCase()}</span>
  }
  ...
  if (loading) return (
    <div style={{minHeight:'100vh',background:'#0c0c0e',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <p style={{color:'#7a7876',fontFamily:'monospace',fontSize:'13px'}}>Cargando FOMO...</p>
    </div>
  )

  const S = { // styles
    page: {padding:'14px 14px 80px',fontFamily:"'Syne',sans-serif"},
    sec: {fontSize:'10px',fontWeight:700,letterSpacing:'.14em',color:'#3a3a38',textTransform:'uppercase' as const,margin:'18px 0 10px'},
    card: {background:'#141416',border:'1px solid rgba(255,255,255,0.07)',borderRadius:'12px',padding:'13px',marginBottom:'10px'},
    inp: {width:'100%',background:'#1c1c20',border:'1px solid rgba(255,255,255,0.13)',borderRadius:'8px',color:'#eeecea',fontFamily:'DM Mono,monospace',fontSize:'16px',padding:'11px 12px',outline:'none',boxSizing:'border-box' as const},
    sel: {width:'100%',background:'#1c1c20',border:'1px solid rgba(255,255,255,0.13)',borderRadius:'8px',color:'#7a7876',fontFamily:'DM Mono,monospace',fontSize:'15px',padding:'11px 12px',outline:'none',boxSizing:'border-box' as const,WebkitAppearance:'none' as const},
        ...
        <div style={{padding:'10px 14px 0'}}>
          {vencimientos.filter(v => diasHasta(v.fecha) === 0).length > 0 && (
            <div style={{background:'rgba(255,80,80,0.12)',border:'1px solid rgba(255,80,80,0.2)',borderRadius:'10px',padding:'11px 13px',fontSize:'12px',color:'#ff8080',marginBottom:'8px',lineHeight:1.5}}>
              <strong>HOY vencen: </strong>{vencimientos.filter(v=>diasHasta(v.fecha)===0).map(v=>v.descripcion).join(', ')} — {fmt(vencimientos.filter(v=>diasHasta(v.fecha)===0).reduce((s:number,v:any)=>s+v.monto,0))}
            </div>
          )}
        ...
        <div style={S.page}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'9px',marginBottom:'14px'}}>
            {[
              ...
              {label:'MES ACTUAL',val:fmtS(datosDia.ventas_acumuladas_mes),color:'#5b9fff',sub:`${new Date().getDate()} días · ${fmtS(datosDia.ventas_acumuladas_mes/new Date().getDate())}/día`,prog:0},
            ].map((k,i) => (
      ...
      {tab === 'cargar' && (
        <div style={S.page}>
          ...
          {[
            ...
            {label:'Tarjeta pendiente de acreditación ($)', key:'tarjeta_pendiente', hint:'Lo que el banco todavía no acreditó — lo cargás cuando lo ves en el resumen bancario'},
            ...
            <div key={f.key} style={S.card}>
              ...
              <input type="number" inputMode="numeric" style={S.inp}
                value={(datosDia as any)[f.key] || ''}
                placeholder="0"
                onChange={e => setDatosDia({...datosDia, [f.key]: parseFloat(e.target.value)||0})}
              />
              {(f as any).hint && <p style={{fontSize:'10px',color:'#3a3a38',marginTop:'5px',fontFamily:'monospace'}}>{(f as any).hint}</p>}
            </div>
          ))}

          <div style={S.card}>
            <label style={S.label}>Notas del día (opcional)</label>
            <input type="text" style={S.inp} placeholder="ej: día lento, falta stock iPhones..."
            ...
            {vencimientos.map((v:any,i:number) => {
              ...
              return (
                <div key={i} style={{padding:'11px 13px',borderBottom:'1px solid rgba(255,255,255,0.07)',display:'flex',alignItems:'center',gap:'8px'}}>
                  ...
                  <button style={{background:'none',border:'none',color:'#3a3a38',cursor:'pointer',fontSize:'16px',padding:'4px'}} onClick={()=>marcarPagado(v.id, v.descripcion)} title="Marcar pagado">✓</button>
                </div>
              )
            })}
          ...
          <div style={{background:'#141416',borderRadius:'16px 16px 0 0',padding:'20px 16px 36px',width:'100%',borderTop:'1px solid rgba(255,255,255,0.13)'}}>
            ...
            {[
              ...
              <div key={f.key} style={{marginBottom:'9px'}}>
                <label style={S.label}>{f.label}</label>
                <input type={f.type} inputMode={f.type==='number'?'numeric':undefined} style={S.inp} placeholder={(f as any).placeholder||''}
                  value={(fVenc as any)[f.key]} onChange={e=>setFVenc({...fVenc,[f.key]:e.target.value})}/>
              </div>
            ))}
          ...
          <div style={{background:'#141416',borderRadius:'16px 16px 0 0',padding:'20px 16px 36px',width:'100%',borderTop:'1px solid rgba(255,255,255,0.13)'}}>
            ...
            {[
              ...
              <div key={f.key} style={{marginBottom:'9px'}}>
                <label style={S.label}>{f.label}</label>
                <input type={f.type} inputMode={f.type==='number'?'numeric':undefined} style={S.inp} placeholder={(f as any).placeholder}
                  value={(fDeuda as any)[f.key]} onChange={e=>setFDeuda({...fDeuda,[f.key]:e.target.value})}/>
              </div>
            ))}
            <div style={{marginBottom:'9px'}}>
              <label style={S.label}>Tipo</label>
